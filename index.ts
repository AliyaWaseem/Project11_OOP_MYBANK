  ...
  do{
    let service = await inquirer.prompt({
      ...
      if (service.service == "Check balance") {
        ...
        let account = UBL.account.find(
          (acc) => acc.accNumber == parseInt(res.number)
        );
        if (!account) {
          console.log(chalk.red.bold("Invalid Account Number"));
        }
        if (account) {
          let name = UBL.customer.find(
            (item) => item.accNumber == account?.accNumber
          );
          console.log(
            `Dear ${chalk.green.italic(name?.firstName)} ${chalk.green.italic(
              name?.lastName
            )} Your current balance is: ${chalk.bold.yellowBright(
              `$${account.balance}`
            )}`
          );
        }
        ...
        if (account) {
          ...
          console.log(
            `Dear ${chalk.green.italic(
              UBL.customer.find((item) => item.accNumber == account?.accNumber)
               ?.firstName
            )} ${chalk.green.italic(
              UBL.customer.find((item) => item.accNumber == account?.accNumber)
               ?.lastName
            )} Your new balance is: ${chalk.bold.yellowBright(`$${newBalance}`)}`
          );
        ...
        if (account) {
          ...
          console.log(
            `Dear ${chalk.green.italic(
              ...
              UBL.customer.find((item) => item.accNumber == account?.accNumber)
               ?.lastName
            )} Your remaining balance is: ${chalk.bold.yellowBright(`$${newBalance}`)}`
          );
